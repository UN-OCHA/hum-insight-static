...
(function ($) {
  ...
  function hpc_homepage_plan_table_align_modal() {
    var modalDialog = $(this).find(".modal-dialog");
    modalDialog.css("margin-top", Math.max(0, ($(window).height() - modalDialog.height()) / 2));
  }

  // Attach behaviors.
  Drupal.behaviors.hpc_homepage_plan_table = {
    attach: function(context, settings) {
      if ($('body #prevent-fts-link-modal', context).length == 0) {
        // Add the modal window once.
        let message = Drupal.t('Strategic planning is in progress for this response. The plan page on FTS will be published in due course.');
        let close_button = Drupal.t('Close');
        let modal = `
          <div id='prevent-fts-link-modal' class='modal fade'>
        ...
        $("#prevent-fts-link-modal").on("shown.bs.modal", hpc_homepage_plan_table_align_modal);
        // Align modal when user resize the window
        $(window).on("resize", function(){
            $("#prevent-fts-link-modal:visible").each(hpc_homepage_plan_table_align_modal);
        });
      }

      $('.prevent-fts-link', context).click(function(e) {
        e.preventDefault();
        let plan_name = $(this).parents('tr').find('td:first-child').text();
        $('#prevent-fts-link-modal .modal-title').html(plan_name);
        $('#prevent-fts-link-modal').modal('show');
      });
    }
  }
...
(function ($) {
  ...
  Drupal.hpc_external_site_modal = {};

  // Attach behaviors.
  Drupal.behaviors.hpc_external_site_modal = {
    attach: function(context, settings) {
      $('.site-preview-button a', context).click(function(e) {
        e.preventDefault();

        var image_urls = $(this).data('images').split(',');
        var title = $(this).data('title');

        Drupal.CTools.Modal.show('hpc-style-big');
        $('#modal-title').html(title);

        var swiper_container = document.createElement('div');
        $(swiper_container).addClass('swiper-container');
        ...
        $(swiper_button_prev).addClass('swiper-button-prev');
        $(swiper_button_prev).addClass('swiper-button');
        $(swiper_button_prev).addClass('swiper-button-black');

        // Next button.
        var swiper_button_next = document.createElement('div');
        $(swiper_button_next).addClass('swiper-button-next');
        $(swiper_button_next).addClass('swiper-button');
        $(swiper_button_next).addClass('swiper-button-black');
        ...
        for (i = 0; i < image_urls.length; i++) {
          ...
          $(image_wrapper).addClass('swiper-slide');
          $(image_wrapper).append(image);
          $(swiper_wrapper).append(image_wrapper);
        }
        ...
        $('#modal-content').append(swiper_container);

        var target_url = $(this).data('url');
        $('#modal-content').append('<div class="target-link"><a href="' + target_url + '" target="_blank">' + Drupal.t('Go to website') + '</a></div>');

        $('#modal-content').scrollTop(0);

        Drupal.hpc_external_site_modal.swiper = new Swiper('.swiper-container', {
          loop: false,
          spaceBetween: 10,
          centeredSlides: true,
          keyboard: true,
        ...
        $('.swiper-button-next').click(function(e) {
          Drupal.hpc_external_site_modal.swiper.slideNext();
          Drupal.hpc_external_site_modal.swiper.navigation.update();
        });
        $('.swiper-button-prev').click(function(e) {
          Drupal.hpc_external_site_modal.swiper.slidePrev();
          Drupal.hpc_external_site_modal.swiper.navigation.update();
        });
